<template>
  <div class="cleared-dashboard">
    <!-- 订单列表区域 -->
    <div class="orders-section">
      <div class="section-header">
        <h2 class="section-title">清关完成订单</h2>
      </div>
    ...
    <div class="order-details-section" v-if="selectedOrder">
      ...
      <div class="actions-section">
        <button 
          ...
          @click="deliverOrderAction"
          :disabled="actionLoading"
        >
          {{ actionLoading ? '处理中...' : '确认交付' }}
        </button>
        <button 
...
import { getLogisticsClearedOrders, updateLogisticsOrderStatus } from '../../utils/api.js'
...
const loadOrders = async () => {
  try {
    ...
    const response = await getLogisticsClearedOrders(userId)
    
    if (response.success && response.orders) {
      orders.value = response.orders
    }
  } catch (error) {
    console.error('Failed to load cleared orders:', error)
  } finally {
    loading.value = false
  }
}
...
const deliverOrderAction = async () => {
  if (!selectedOrder.value || actionLoading.value) return
  
  if (!confirm(`确定订单 #${selectedOrder.value.id} 已交付完成吗？`)) {
    return
  }
  ...
  try {
    ...
    const userId = userStore.getUserId()
    const response = await updateLogisticsOrderStatus(selectedOrder.value.id, userId, 'deliver')
