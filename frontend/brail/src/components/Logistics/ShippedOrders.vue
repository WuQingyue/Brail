<template>
  <div class="shipped-dashboard">
    <!-- 订单列表区域 -->
    <div class="orders-section">
      <div class="section-header">
        <h2 class="section-title">运输中订单</h2>
      </div>
    ...
    <div class="order-details-section" v-if="selectedOrder">
      ...
      <div class="actions-section">
        <button 
          class="action-btn customs-btn"
          @click="arriveCustomsAction"
          :disabled="actionLoading"
        >
          {{ actionLoading ? '处理中...' : '到达巴西海关' }}
        </button>
        <button 
...
import { getLogisticsShippedOrders, updateLogisticsOrderStatus } from '../../utils/api.js'
...
const loadOrders = async () => {
  try {
    ...
    const response = await getLogisticsShippedOrders(userId)
...
const arriveCustomsAction = async () => {
  if (!selectedOrder.value || actionLoading.value) return
  
  if (!confirm(`确定货物已到达巴西海关 #${selectedOrder.value.id} 吗？`)) {
    return
  }
  ...
  try {
    ...
    const userId = userStore.getUserId()
    const response = await updateLogisticsOrderStatus(selectedOrder.value.id, userId, 'arrive_customs')
    ...
    console.error('Failed to update customs status:', error)
    alert('更新海关状态失败，请重试。')
  } finally {
...
.customs-btn:hover:not(:disabled) {
