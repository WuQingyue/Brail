<template>
  <div class="customs-dashboard">
    <!-- 订单列表区域 -->
    <div class="orders-section">
      <div class="section-header">
        <h2 class="section-title">到达巴西海关订单</h2>
      </div>
    ...
    <div class="order-details-section" v-if="selectedOrder">
      ...
      <div class="actions-section">
        <button 
          ...
          @click="clearCustomsAction"
          :disabled="actionLoading"
        >
          {{ actionLoading ? '处理中...' : '清关完成' }}
        </button>
        <button 
...
import { getLogisticsCustomsOrders, updateLogisticsOrderStatus } from '../../utils/api.js'
...
const loadOrders = async () => {
  try {
    ...
    const response = await getLogisticsCustomsOrders(userId)
    
    if (response.success && response.orders) {
      orders.value = response.orders
    }
  } catch (error) {
    console.error('Failed to load customs orders:', error)
  } finally {
    loading.value = false
  }
}
...
const clearCustomsAction = async () => {
  if (!selectedOrder.value || actionLoading.value) return
  
  if (!confirm(`确定订单 #${selectedOrder.value.id} 已清关完成吗？`)) {
    return
  }
  ...
  try {
    ...
    const userId = userStore.getUserId()
    const response = await updateLogisticsOrderStatus(selectedOrder.value.id, userId, 'clear_customs')
    ...
    console.error('Failed to clear customs:', error)
    alert('清关操作失败，请重试。')
  } finally {
...
.cleared-btn:hover:not(:disabled) {
